...
<h1 align="center"> Find report form </h1>
...
if (!$db->query($query))
	...
	echo "<h3 align='center'>  As soon as a matching is found we will contact you </h3>";
}
...
$query = sprintf("Select * from lost where resolved is null and username!='".$username."' and detailid=%d ",$detailid);
...
$body_main='';
	...
	$body="Dear ".$row['username']." Your item may have been found. You may be contacted shortly directly by the person who found it";
	$mail->Body = $body;
	...
	$body_main = $body_main.$to." ";
		...
		$query = "select itemid from found order by itemid desc limit 1";
	...
	$from = "as8g10@gmail.com";
	$from_name="LOST AND FOUND, University of Awesome";    
	$subject="Match for your description may have been found";       
	$mail->SetFrom($from, $from_name);
	$mail->Subject = $subject;
	
	$body="Dear ".$username." We may have found a potential owner(s) of the item you uploaded: http://localhost/devx/display_full_found.php?itemid=".$itemid." These are their contact details. PLease contact them at your discretion: ".$body_main;
	$mail->Body = $body;
...
<form name="report" action = "found.php" method="post" >
